...
Production-контур состоит из PostgreSQL, FastAPI backend и Nginx, который раздает собранную веб-панель, завершает TLS и проксирует REST/WebSocket на backend.

## 1. Подготовить переменные

Скопируйте пример без секретов:
...
Каталог `secrets/` исключен из Git. Пример подготовки:
...
JWT-секрет должен содержать минимум 32 символа. Production backend дополнительно проверяет, что:

- используется PostgreSQL;
- `CEH_AUTO_CREATE_SCHEMA=false`;
- JWT-секрет не является тестовым/дефолтным.
...
Backend перед стартом выполняет:

```text
alembic upgrade head
```

и только после успешной миграции запускает Uvicorn.
...
PostgreSQL и backend имеют контейнерные healthcheck. Внешний доступ к PostgreSQL и порту backend не публикуется: наружу открыты только Nginx 80/443.
...
- ключ 1С должен быть отдельным от JWT и пароля PostgreSQL;
...
Текущий WebSocket broadcaster живет в памяти одного процесса. Перед горизонтальным масштабированием backend на несколько экземпляров необходимо вынести fan-out событий в Redis или другой общий broker. До этого production compose намеренно запускает один экземпляр backend.
