...
def build_env(ip: str, port: int, admin_login: str) -> str:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Безопасно создать .env.production для Цех Склад по публичному IP и нестандартному HTTPS-порту"
    )
    parser.add_argument("--ip", required=True, help="Публичный IPv4/IPv6 адрес NAT/production")
    parser.add_argument("--port", required=True, type=int, help="Внешний HTTPS-порт приложения, например 40443")
...
def main() -> int:
    ...
    secure_write(output, build_env(ip, port, admin_login))
    ...
    print("TLS: внутренний CA Caddy; внешние TCP 80/443 для сертификата не требуются.")
    print("После первого запуска экспортируйте root CA и установите его на доверенные клиентские устройства.")
