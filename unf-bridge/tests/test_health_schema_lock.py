...
def test_release_health_is_degraded_until_schema_lock_is_accepted():
    health = check_health(
    ...
    assert health.expected_metadata_structure_sha256 is None
    assert health.metadata_structure_matches_expected is False


def test_release_health_is_ready_with_matching_schema_lock():
    fresh = FakeFresh()
    digest = metadata_structure_sha256(URL, fresh.entity_sets())
    mapping = TenantMapping.from_dict(
        {**BASE, "expected_metadata_structure_sha256": digest}
    )
    ...
    assert health.expected_metadata_structure_sha256 == digest
    assert health.metadata_structure_sha256 == digest
    assert health.metadata_structure_matches_expected is True
