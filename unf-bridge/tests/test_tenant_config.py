...
from unf_bridge.odata import ODataEntitySet, ODataNavigation
...
def test_tenant_mapping_validates_resources_document_fields_and_price_fields():
    mapping = TenantMapping.from_dict(VALID); mapping.validate_against_metadata(metadata_sets())
    assert mapping.price_fields["value"] == "Цена"


def test_tenant_mapping_accepts_nested_price_path_when_root_is_navigation():
    payload = {**VALID, "price_fields": {**VALID["price_fields"], "product_ref": "Номенклатура/Ref_Key"}}
    ...
    sets = [
        ODataEntitySet(
            item.name,
            item.entity_type,
            properties=("ВидЦен_Key", "Цена"),
            navigation=(ODataNavigation("Номенклатура", "StandardODATA.Catalog_Номенклатура"),),
...
def test_tenant_mapping_rejects_unsafe_path_and_nested_external_key():
    payload = {**VALID, "price_fields": {**VALID["price_fields"], "product_ref": "Номенклатура) or true"}}
    with pytest.raises(ValueError, match="путь"):
        TenantMapping.from_dict(payload)
    payload = {**VALID, "external_key_fields": {**VALID["external_key_fields"], "sale": "Комментарий/Value"}}
    with pytest.raises(ValueError, match="прямым"):
        TenantMapping.from_dict(payload)


def test_tenant_mapping_requires_price_fields_and_two_distinct_price_types():
    payload = {**VALID, "price_fields": {**VALID["price_fields"]}}; del payload["price_fields"]["value"]
    with pytest.raises(ValueError, match="value"): TenantMapping.from_dict(payload)
    payload = {**VALID, "constants": {**VALID["constants"]}}; payload["constants"]["wholesale_price_type_ref"] = payload["constants"]["retail_price_type_ref"]
    with pytest.raises(ValueError, match="разными"): TenantMapping.from_dict(payload)


def test_tenant_mapping_detects_wrong_price_field_missing_resource_and_wrong_guid():
    mapping = TenantMapping.from_dict(VALID); sets = metadata_sets(); price_name = VALID["resources"]["prices"]
    bad = [ODataEntitySet(x.name, x.entity_type, ("Номенклатура_Key", "ВидЦен_Key", "Стоимость")) if x.name == price_name else x for x in sets]
    with pytest.raises(ValueError, match=r"prices: .*\.Цена"): mapping.validate_against_metadata(bad)
    with pytest.raises(ValueError, match="Списание"): mapping.validate_against_metadata([x for x in sets if x.name != VALID["resources"]["stock_writeoff"]])
    payload = {**VALID, "constants": {**VALID["constants"], "organization_ref": "not-a-guid"}}
    with pytest.raises(ValueError, match="organization_ref"): TenantMapping.from_dict(payload)
